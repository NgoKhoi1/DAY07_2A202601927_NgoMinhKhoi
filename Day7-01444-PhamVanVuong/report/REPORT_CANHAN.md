...
> **Lưu ý:** tại thời điểm viết báo cáo này, `REPORT_NHOM.md` của nhóm vẫn ở dạng template (bộ tài liệu trong `data/k4_ecommerce/` mới chỉ là 2 tài liệu khởi động với `source_url` kiểu `example.com`, chưa phải nguồn công khai thật, và mục 3 "Câu hỏi đánh giá" của nhóm chưa được điền). Vì vậy 5 câu hỏi dưới đây là **bộ câu hỏi nháp cá nhân** tôi tự đặt trên dữ liệu khởi động để kiểm tra pipeline cá nhân; khi nhóm chốt 5 câu hỏi chính thức trong `REPORT_NHOM.md`, tôi sẽ chạy lại và cập nhật bảng này cho khớp. Cấu hình chạy: `RecursiveChunker(chunk_size=300)` + `MockEmbedder` (mặc định, chưa có API key) + `demo_llm` (hàm LLM giả lập trong `main.py`, chỉ echo lại đoạn đầu prompt).

| # | Câu hỏi (Query)                                         | Top-1 Chunk truy xuất được (tóm tắt)                                                                                 | Điểm Score | Có liên quan không? (Relevant)                                                                                                                      | Câu trả lời của Agent (tóm tắt)                                             |
| - | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------- |
| 1 | Người mua cần làm gì để đổi trả hàng lỗi?     | "Người bán có trách nhiệm phản hồi theo quy trình của sàn..." (returns-policy)                                  | -0.0789      | Không (chunk nêu đúng nghĩa vụ của người mua không lọt top-3)                                                                               | [DEMO] echo context ở trên (chưa có LLM thật để tổng hợp câu trả lời) |
| 2 | Thời hạn gửi yêu cầu đổi trả là bao lâu?        | Đoạn ghi chú metadata mẫu của returns-policy                                                                          | 0.0694       | Không (chunk chứa "trong thời hạn được nêu trên trang sản phẩm..." không lọt top-3)                                                       | [DEMO] echo context ở trên                                                      |
| 3 | Người bán có trách nhiệm gì khi đăng sản phẩm? | Đoạn ghi chú metadata mẫu của seller-listing                                                                          | 0.0644       | Không (chunk "Người bán chịu trách nhiệm cung cấp thông tin sản phẩm chính xác..." không lọt top-3)                                     | [DEMO] echo context ở trên                                                      |
| 4 | Sản phẩm nào không được phép đăng bán?         | "Người mua cần gửi yêu cầu đổi trả trong thời hạn..." (returns-policy, sai chủ đề)                           | 0.2340       | Không (chunk "Sản phẩm bị hạn chế hoặc bị cấm..." không lọt top-3)                                                                          | [DEMO] echo context ở trên                                                      |
| 5 | Ai chịu trách nhiệm xử lý yêu cầu đổi trả?      | "Người bán chịu trách nhiệm cung cấp thông tin sản phẩm chính xác..." (seller-listing, liên quan gián tiếp) | 0.1676       | Một phần — chunk đúng chủ đề ("Người bán có trách nhiệm phản hồi theo quy trình của sàn...") có xuất hiện nhưng chỉ ở hạng 3 | [DEMO] echo context ở trên                                                      |

**Bao nhiêu câu hỏi trả về chunk có liên quan trong top-3?** 1 / 5 (chỉ câu 5, và cũng ở hạng 3 chứ không phải top-1)
...
> Tự trừ điểm ở mục 5 vì bộ tài liệu và bộ câu hỏi đánh giá của nhóm chưa được chốt (`REPORT_NHOM.md` còn là template) — kết quả retrieval hiện tại chỉ mang tính minh họa trên dữ liệu khởi động + MockEmbedder, cần chạy lại khi nhóm hoàn thiện Giai đoạn 1 (chọn nguồn thật) để có điểm truy xuất phản ánh đúng năng lực pipeline.
